...
import { Ban, RotateCcw, CheckCircle2, ArrowUpRight, Users, Trash2, Link2 } from "lucide-react";
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {!partner.user.deletedAt && (
          <form action={deleteUserAccount}>
            <input type="hidden" name="userId" value={partner.user.id} />
            <input type="hidden" name="redirectTo" value="/admin/partners" />
            <ConfirmSubmit
              message={`Delete ${partner.user.name}'s account? Their email frees up for a new signup immediately. Their referral code and commission history stay on file, just suspended — reattaching that specific referral history to a new account isn't supported yet, only past bookings/Client records are.`}
      ...
      {!partner.user.deletedAt && (
        <div className="glass mt-8 rounded-2xl p-6">
          ...
          <p className="mt-1 max-w-lg text-sm text-[var(--color-slate)]">
            If this person had a client account before that was deleted, find it by the
            phone number or email it used — their past bookings (and their old Client
            record, if it&apos;s not already spoken for) reattach to this account. Doesn&apos;t
            cover referral-partner history — see the note above.
          </p>
          <form action={linkPastAccount} className="mt-4 flex flex-wrap gap-2">
            <input type="hidden" name="newUserId" value={partner.user.id} />
            <input type="hidden" name="redirectTo" value={`/admin/partners/${id}`} />
            <input
              name="lookup"
