...
export function ProjectFileUpload({
  projectId,
  addFileAction,
  folder = "nobs-agent/project-files",
}: {
  projectId: string;
  ...
  async function handleFile(e: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      // resource_type=auto (not a signed param) lets this endpoint accept
      // PDFs, docs, and images alike, contracts and briefs, not just photos.
      ...
      attachData.append("projectId", projectId);
      attachData.append("url", uploadJson.secure_url);
      attachData.append("fileName", file.name);
      await addFileAction(attachData);
    } catch (err) {
