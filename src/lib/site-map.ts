export type SiteMapAccess = "public" | "client" | "middleman" | "admin";
...
// detail pages (a specific blog post, project, booking, etc.) aren't
// enumerable here without a data fetch, so this covers every static
// route and list/section page, powering the global search so a signed-in
// user can jump straight to any page or sub-page they have access to.
export const siteMap: SiteMapEntry[] = [
  ...
  { label: "Middleman sign up", href: "/middleman/signup", section: "Account", access: "public" },
  ...
  // ---- Middleman ----
  { label: "Middleman dashboard", href: "/middleman/dashboard", section: "Middleman", access: "middleman" },
...
export function siteMapFor(role: "ADMIN" | "STAFF" | "CLIENT" | "MIDDLEMAN" | undefined): SiteMapEntry[] {
  return siteMap.filter((entry) => {
    ...
    if (entry.access === "middleman") return role === "MIDDLEMAN";
    return false;
  });
}
